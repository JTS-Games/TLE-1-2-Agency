<x-layout>
    ...
    <div class="bg-[rgb(251,252,247)] min-h-screen flex items-center justify-center py-12 px-4">
        <div class="bg-[rgb(226,236,200)] p-8 rounded-lg shadow-md w-full max-w-4xl">
            ...
            <h1 class="text-[rgb(170,1,96)] font-bold text-3xl mb-8 font-radikal-bold text-center">Nieuwe Vacature
                Maken</h1>
            ...
            <form action="{{ route('vacancies.store') }}" method="post" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="file" id="image" name="image"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>


                    <div>
                        <label for="job_title" class="block font-radikal text-[rgb(49,61,41)]">Functie:</label>
                        <input type="text" id="job_title" name="job_title" placeholder="Vul functie in"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>


                    <div>
                        <label for="description" class="block font-radikal text-[rgb(49,61,41)]">Functie
                            omschrijving:</label>
                        <textarea id="description" name="description" rows="3"
                                  placeholder="Voeg hier extra informatie toe"
                                  class="mt-1 block w-full border border-black rounded-lg p-2"></textarea>
                    </div>


                    <div>
                        <label for="location" class="block font-radikal text-[rgb(49,61,41)]">Locatie:</label>
                        <input type="text" id="location" name="location" placeholder="Voer de locatie in"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="paycheck" name="paycheck" placeholder="Voer loon in"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="working_hours" name="working_hours" placeholder="Voer werkuren in"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>
                    ...
                    <div>
                        <label for="contract_term"
                               class="block font-radikal text-[rgb(49,61,41)]">Dienstverband:</label>
                        <input type="text" id="contract_term" name="contract_term" placeholder="Voer dienstverband in"
                               class="mt-1 block w-full border border-black rounded-lg p-2">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="qualifications" name="qualifications[]" multiple
                        ...
                        @foreach($qualifications as $qualification)
                            <option
                                value="{{ $qualification->id }}" {{ collect(old('qualifications'))->contains($qualification->id) ? 'selected' : '' }}>
                                {{ $qualification->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex justify-center">
                    <button type="submit" class="px-6 py-2 rounded-lg border-2"
                            style="background-color: #AA0160; color: white;">
                        Maak Vacature
                    </button>
                </div>
            </form>
